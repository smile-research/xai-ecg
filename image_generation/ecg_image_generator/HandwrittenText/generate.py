...
def get_handwritten(link,num_words,input_file,output_dir,x_offset=0,y_offset=0,handwriting_size_factor=0.2,model_path=os.path.join(os.path.join('./ecg_image_generator/HandwrittenText','pretrained'), 'model-29'),text=None,style=None,bias=1.,force=False,animation=False,noinfo=True,save=None,bbox= False, output_path=None):
    ...
    if(validators.url(link)):
        ...
        if link == '':
            link = './ecg_image_generator/HandwrittenText/Biomedical.txt'
    ...
    with open(os.path.join(os.path.join('./ecg_image_generator/HandwrittenText','data'), 'translation.pkl'), 'rb') as file:
    ...
    with tf.compat.v1.Session(config=config) as sess:
        ...
        final_path = output_path if output_path else os.path.join(output_dir,tail)
