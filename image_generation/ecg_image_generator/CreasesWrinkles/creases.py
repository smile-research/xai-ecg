...
def get_creased(input_file,output_directory,ifWrinkles=False,ifCreases=False,crease_angle=0,num_creases_vertically=3,num_creases_horizontally=2,bbox=False, output_path=None):
    filename =  input_file

    if(ifWrinkles):
    #Seed with a different selection of a wrinkle image
    # read wrinkle image as grayscale and convert to float in range 0 to 1
        wrinkle_file_name = os.path.join(os.path.join('./ecg_image_generator/CreasesWrinkles','wrinkles-dataset') , random.choice(os.listdir(os.path.join('./ecg_image_generator/CreasesWrinkles','wrinkles-dataset'))))
    ...
    # save results
    filename = output_path if output_path else filename
